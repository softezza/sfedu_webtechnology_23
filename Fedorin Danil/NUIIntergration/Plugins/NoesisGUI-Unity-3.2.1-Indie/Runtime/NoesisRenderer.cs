...
public class NoesisRenderer
{
    ...
    public static void RenderOnscreen(Noesis.View view_, Noesis.Matrix4 projection_, bool flipY,
        UnityEngine.Rendering.CommandBuffer commands, bool invalidate)
    {
        FixMetalEncoder(commands);
    ...
    public static System.IntPtr CreatePixelShader(byte shader, int numTextures, byte[] bytes)
    {
        if (bytes != null)
        {
            ...
            _commands.IssuePluginEventAndData(_renderCreateShaderCallback, (int)EventId.NoRender, ptr);
