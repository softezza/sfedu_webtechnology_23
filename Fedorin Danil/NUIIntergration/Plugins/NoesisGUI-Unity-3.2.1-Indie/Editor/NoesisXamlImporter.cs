...
class NoesisXamlImporter : ScriptedImporter
{
    ...
    private static void AddRive(string uri, ref List<NoesisRive> rives)
    {
        if (!String.IsNullOrEmpty(AssetDatabase.AssetPathToGUID(uri)))
        {
    ...
    private static string ScanDependencies(AssetImportContext ctx, out List<NoesisFont> fonts_,
        ...
        out List<NoesisRive> rives_, out List<NoesisXaml> xamls_,
        out List<NoesisShader> shaders_)
    {
    ...
    public override void OnImportAsset(AssetImportContext ctx)
    {
        ...
        string root = ScanDependencies(ctx, out fonts, out textures, out sprites, out audios, out videos, out rives, out xamls, out shaders);
