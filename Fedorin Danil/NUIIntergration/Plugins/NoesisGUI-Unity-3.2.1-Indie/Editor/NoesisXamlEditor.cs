...
public class NoesisXamlEditor: Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        _showRives = EditorGUILayout.Foldout(_showRives, $"Rives ({xaml.rives.Length})", false);
        if (_showRives && xaml.rives != null)
        {
            foreach (var rive_ in xaml.rives)
            {
                EditorGUILayout.ObjectField(rive_.rive, typeof(NoesisRive), false);
            }
        }
            ...
            if (CanRender())
            {
                if (r.width > 4 && r.height > 4)
                {
                    if (_viewPreview != null && _viewPreview.Content != null)
                    {
                        if (_baseTime == 0.0)
                        {
                            _baseTime = EditorApplication.timeSinceStartup;
                        }

                        double time = EditorApplication.timeSinceStartup - _baseTime;
                        UnityEngine.GUI.DrawTexture(r, RenderPreview(_viewPreview, (int)r.width, (int)r.height, time));
                    }
                }
            }
        }
    ...
    private RenderTexture RenderPreview(Noesis.View view, int width, int height, double time)
    {
        try
        {
